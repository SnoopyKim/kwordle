...
class LetterView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (!_provider.isReadyToInput && _provider.inputHistory.isEmpty) {
      Timer(const Duration(milliseconds: 300), () => _provider.setReadyToInput(true));
    }
    return Column(
      mainAxisAlignment: MainAxisAlignment.center,
      children: List.generate(_provider.inputHistory.length,
          (i) => LetterRow(length: _provider.mode, letterList: _provider.inputHistory[i]))
        ..add(AnimatedContainer(
          duration: Duration(milliseconds: _provider.isReadyToInput ? 300 : 0),
          transform: _provider.isReadyToInput
              ? Matrix4.identity()
              : Matrix4.translationValues(MediaQuery.of(context).size.width * -1, 0, 0),
          child: LetterRow(
...
class LetterRow extends StatelessWidget {
  const LetterRow({Key? key, required this.length, required this.letterList}) : super(key: key);
  final int length;
  final List<Map<String, dynamic>> letterList;

  @override
  Widget build(BuildContext context) {
    final boxSize = (MediaQuery.of(context).size.width - 40 - 6 * (length - 1)) / length;
    return Row(
        mainAxisAlignment: MainAxisAlignment.center,
        children: List.generate(length, (i) {
          Map<String, dynamic> data = i < letterList.length ? letterList[i] : {};
          return LetterBox(index: i, size: boxSize, letter: data['letter'], result: data['result']);
        }));
  }
}
